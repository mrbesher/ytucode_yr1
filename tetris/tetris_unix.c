...
int main() {
  srand(time(0));
  //shapes declaration
  int random,randomcolor,map_size=2,maxbottom,minbottom,row_check,col_check,resume=1,maxscore=0,score=0,gameover=0,counter=0,counter2,right,left,dropcolumn,stop,rotation,j,k;
  int map[50][50],shapes[9][4][4],rows2delete[4],origin[2],bottom[4];
  char ans[2],colors[7][8]={
    "\x1b[31m", //red
    "\x1b[32m", //green
    "\x1b[33m", //yellow
    "\x1b[34m", //blue
  ...
  printf("Ready? (y/n): ");scanf("%s",&ans[0]);printf("\033[H\033[J");
  if (ans[0]=='y') {
  ...
  while (gameover==0) {
    ...
    printf("%s",colors[randomcolor+1]);
    for (size_t i = 0; i < 4; i++) {
      for (size_t k = 0; k < 4; k++) {
        if (shapes[0][i][k]==1) {
          printf("+ ");
        } else if (shapes[0][i][k]==2) {
          printf("%s+ %s",colors[0],colors[randomcolor+1]);
        } else {
          printf("  ");
        }
      }
    ...
    //print the map
    printf("\033[H\033[J");
    printf("\n**\x1b[31mT\x1b[0m.\x1b[32mE\x1b[0m.\x1b[33mT\x1b[0m.\x1b[34mR\x1b[0m.\x1b[35mI\x1b[0m.\x1b[36mS\x1b[0m**\n");
    ...
    for (size_t i = 1; i < map_size; i++) {
      for (size_t k = 0; k < map_size; k++) {
        if (map[i][k]==0) {
          ...
          printf("%s+ %s",colors[map[i][k]],colors[6]);
        }
      }
      printf("\n");
    }
    ...
    while (counter==0) {
      ...
      if (map_size-dropcolumn>=right && dropcolumn>left) {
        ...
        printf("\n%sE: The shape exceeds the map if dropped from there.\nAre you sure that you checked the map?%s\n\n",colors[0],colors[6]);
      }
    }
    //the exciting part :)
    ...
    for (size_t i = 0; i <bottom[maxbottom]+1; i++) {
      for (size_t k = dropcolumn-left; k < dropcolumn+right+1; k++) {
        if (shapes[0][bottom[maxbottom]-i][counter+k-dropcolumn+left]!=0) {
          map[counter2-1-i][k-1]=randomcolor+1;
        }
      }
      printf("\n");
    }
